...
function onMouseDown(event) {
    ...
    if (highlightedObject) {
        ...
        console.log(`Clique detectado no objeto com ID: ${id}`);

        const { branch } = findNodeById(treeData, id);

        if (branch) {
            console.log(`Galho encontrado para poda. ID do galho: ${branch.id}, Nível: ${branch.level}`);
            ...
            console.log("IDs a serem podados:", prunedIds);

            buildTreeView();
            highlightedObject = null;
        } else {
            console.log("Nenhum galho correspondente encontrado nos dados da árvore.");
        }
    }
}
